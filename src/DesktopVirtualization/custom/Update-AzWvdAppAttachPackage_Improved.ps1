function Update-AzWvdAppAttachPackage_Improved {
    ...
    param(
        ...
        [Parameter(HelpMessage = 'Make this version of the package the active one across the hostpool.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.Management.Automation.SwitchParameter]
        # Make this version of the package the active one across the hostpool.
        ${IsActive},
    
        [Parameter(HelpMessage = 'Specifies how to register Package in feed.')]
        ...
        [Parameter(HelpMessage = 'Date Package was last updated, found in the appxmanifest.xml.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.DateTime]
        ...
        [Parameter(HelpMessage = 'Package Family Name from appxmanifest.xml; contains package name and publisher name')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.String]
        ...
        [Parameter(HelpMessage = 'Package Full Name from appxmanifest.xml.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.String]
        # Package Full Name from appxmanifest.xml.
        ${PackageFullName},
    
        [Parameter(HelpMessage = 'Package Name from appxmanifest.xml.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.String]
        # Package Name from appxmanifest.xml.
        ${PackageName},
    
        [Parameter(HelpMessage = 'Relative Path to the package inside the image.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.String]
        ...
        [Parameter(HelpMessage = 'Package Version found in the appxmanifest.xml.')]
        [Microsoft.Azure.PowerShell.Cmdlets.DesktopVirtualization.Category('Body')]
        [System.String]
    ...
    process {
        ...
        $UpdateParameters = @("FailHealthCheckOnStagingFailure", "HostPoolReference", "KeyVaultUrl", "ImagePath", "Tag")
        $matchingKeys = $PSBoundParameters.Keys.Where({$UpdateParameters -contains $_})

        $finalParameters = @{}
        
        if($PSBoundParameters.ContainsKey("CertificateExpiry") -and $null -ne $PSBoundParameters["CertificateExpiry"]) {
        ...
        if($PSBoundParameters.ContainsKey("LastUpdated") -and $null -ne $PSBoundParameters["LastUpdated"]) {
        ...
        # This shoud be true if none of the object fields are being updated but the permissions variables are set
        # In this case we will get the app attach package to ensure it exists and then update the permissions
        $updatePermissionsOnly = ($matchingKeys.Count -eq 0) -and ($finalParameters.Count -eq 0) `
             -and (($null -ne $PermissionsToRemove) -or ($null -ne $PermissionsToAdd))

        foreach ($key in $PSBoundParameters.Keys) {
            $finalParameters.Add($key, $PSBoundParameters[$key])
        }
        if ($updatePermissionsOnly) {
            $null = $finalParameters.Remove("Location")
            $appAttachPackage = Get-AzWvdAppAttachPackage @finalParameters
        } else {
