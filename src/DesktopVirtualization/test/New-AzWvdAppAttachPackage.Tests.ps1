...
Describe 'New-AzWvdAppAttachPackage' {
    ...
    It 'Improved' {
        try { 
            ...
            $packages[0].ImagePackageName | Should -Be 'MsixUnitTest_Name'
            $packages[0].ImagePackageRelativePath | Should -Be 'MsixUnitTest_RelativePackageRoot'
            $packages[0].FailHealthCheckOnStagingFailure | Should -Be 'Unhealthy'
         } 
         finally{
            $package = Remove-AzWvdAppAttachPackage -Name 'TestPackage' `
                -ResourceGroupName $env.ResourceGroup `
    ...
    It 'MissingHealthCheckAction' {
        try { 
            ...
            $packages[0].ImagePackageName | Should -Be 'MsixUnitTest_Name'
            $packages[0].ImagePackageRelativePath | Should -Be 'MsixUnitTest_RelativePackageRoot'
            $packages[0].FailHealthCheckOnStagingFailure | Should -Be 'NeedsAssistance'
         } 
         finally{
            $package = Remove-AzWvdAppAttachPackage -Name 'TestPackage' `
                -ResourceGroupName $env.ResourceGroup `
    ...
    It 'ImageObjectListPipeline' {
        try {
            ...
            ,$imageList | New-AzWvdAppAttachPackage -Name "TestPackage" `
                -ResourceGroupName $env.ResourceGroup `
                -SubscriptionId $env.SubscriptionId `
                -Location $env.Location `
                -FailHealthCheckOnStagingFailure 'Unhealthy' 
