...
Describe 'Get-AzWvdAppAttachPackage' {
    It 'List' {
        try {
            ...
            $packages[1].ImagePackageFamilyName | Should -Be  'MsixUnitTest_FamilyName2'
            $packages[1].ImagePath | Should -Be 'C:\\msix\SingleMsix.vhd'
            ($packages[1].ImagePackageApplication | ConvertTo-Json) | Should -Be ($apps | ConvertTo-Json)
            ($packages[1].ImagePackageDependency | ConvertTo-Json) | Should -Be ($deps | ConvertTo-Json)
            $packages[1].ImagePackageName | Should -Be 'MsixUnitTest_Name2'
            $packages[1].ImagePackageRelativePath | Should -Be 'MsixUnitTest_RelativePackageRoot2'
    
        }
        finally{
            $package = Remove-AzWvdAppAttachPackage -Name 'TestPackage1' `
                -ResourceGroupName $env.ResourceGroup `
                -SubscriptionId $env.SubscriptionId 
